...
class GNNAgent(BaseAgent):
    ...
    def recommend(
            ...
            extra_simulation_budget: int = 300,
    ) -> List[dict]:
        """Propose several suitable actions for a single observation.

        The first entry is the action :meth:`act_with_id` would pick: the model
        ranking is searched with the same unrestricted budget, up to
        ``max_action_sim`` candidates, until one simulates at or below
        ``best_action_threshold``. Only once that action is found does the
        search become bounded: it continues from the same position for at most
        ``extra_simulation_budget`` further simulations to gather the remaining
        proposals.

        If no candidate ever reaches the threshold, the whole ranking has been
        simulated anyway and the candidates reducing rho the most are proposed
        instead, the first of which is again what :meth:`act_with_id` returns.

        Unlike :meth:`act_with_id` this method holds no state between calls, so
        it is safe for serving independent requests.

        Args:
            observation: Grid2Op observation.
            n_recommendations: Maximum number of actions to propose.
            extra_simulation_budget: Simulations allowed after the first
                suitable action has been found.

        Returns:
            A list of dicts, best first. Each holds the Grid2Op ``action``, its
            ``action_id`` and ``rank`` in the model ranking, the model
            ``probability``, the ``simulated_rho`` it achieves, and whether it
            met the threshold (``suitable``).

        """
        baseline = float(observation.rho.max())

        # Secure state: mirror the low-threshold branch of act_with_id.
        if baseline < self.best_action_threshold:
            if self.topo:
                default_action = self.action_space.from_vect(
                    revert_topo(self.action_space, observation, rho_limit=0.8))
            else:
                default_action = self.action_space({})
            return [{"action": find_best_line_to_reconnect(obs=observation,
                                                           original_action=default_action),
                     "action_id": -1, "rank": -1, "probability": None,
                     "simulated_rho": baseline, "suitable": True}]

        probabilities = self.action_probabilities(observation)
        sorted_actions = probabilities.argsort()[::-1][:self.max_action_sim]

        suitable, improving = [], []
        budget_left = None

        for rank, idx in enumerate(sorted_actions):
            rho_max, valid_action = simulate_action(
                action_space=self.action_space, obs=observation,
                action_vect=self.actions[idx, :], check_overload=self.check_overload,
            )
            ...
            if valid_action:
                self.total_valid_simulation_times += 1
                candidate = {"action_id": int(idx), "rank": rank,
                             "probability": float(probabilities[idx]),
                             "simulated_rho": float(rho_max)}

                if rho_max <= self.best_action_threshold:
                    suitable.append(candidate)
                    if budget_left is None:
                        # The action act_with_id would have returned. Everything
                        # simulated from here on only buys the extra proposals.
                        budget_left = extra_simulation_budget
                    if len(suitable) >= n_recommendations:
                        break
                elif rho_max < baseline:
                    improving.append(candidate)

            if budget_left is not None and budget_left <= 0:
                break

        # Suitable candidates keep discovery order, so the agent's own choice
        # stays first. Fallbacks are ranked by how far they push rho down.
        chosen = suitable or sorted(improving, key=lambda c: c["simulated_rho"])

        recommendations = []
        for candidate in chosen[:n_recommendations]:
            action = next(split_action_and_return(
                observation, self.action_space, self.actions[candidate["action_id"], :]))
            candidate["action"] = find_best_line_to_reconnect(obs=observation,
                                                             original_action=action)
            candidate["suitable"] = candidate["simulated_rho"] <= self.best_action_threshold
            recommendations.append(candidate)

        if not recommendations:
            # Nothing helped at all, so fall back to do-nothing as act_with_id does.
            recommendations.append(
                {"action": find_best_line_to_reconnect(obs=observation,
                                                       original_action=self.action_space({})),
                 "action_id": -1, "rank": -1, "probability": None,
                 "simulated_rho": baseline, "suitable": False})
