...
from evaluation.utilities import simulate_action, find_best_line_to_reconnect, revert_topo
from evaluation.general_tutor import GeneralTutor, print_status
...
class NminusOneTutor(GeneralTutor):
    ...
    def act_with_id_return_all(self, observation: BaseObservation) -> Tuple[np.ndarray, int]:
        """In this methode, we execute the N-1 Action search.
        ...
        if self.__n_minus_one_search_possible(obs=observation):
            ...
            for idx, action_array in self.actions[0].items():
                ...
                action_rho_list.append((idx, current_rho_max_max))
            ...
            if action_chosen is not None:
                ...
                action_rho_list.append((action_chosen, rho_max))
        ...
        if action_chosen is not None:
            out = action_chosen, best_action_index, action_rho_list

        else:
            out = self.action_space({}).to_vect(), -1, action_rho_list
